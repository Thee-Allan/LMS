...
import { Plus, Search, Download, Eye, Trash2, X, FileText, File, FolderOpen, Upload, Tag, Lock, Globe, Users } from 'lucide-react';
...
const DocumentsModule: React.FC = () => {
  ...
  const [form, setForm] = useState({ name: '', category: 'other' as Document['category'], accessLevel: 'team' as Document['accessLevel'], tags: '' });
  ...
  const handleUpload = () => {
    ...
    const newDoc: Document = {
      id: `d${Date.now()}`, name: form.name, type: form.name.split('.').pop() || 'pdf', size: `${(Math.random() * 5 + 0.1).toFixed(1)} MB`,
      uploadedBy: 'Current User', uploadedAt: new Date().toISOString().split('T')[0],
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {hasPermission('documents.create') && (
          <button onClick={() => { setForm({ name: '', category: 'other', accessLevel: 'team', tags: '' }); setShowUpload(true); }}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowUpload(false)}>
          <div className="bg-[var(--card-bg)] rounded-xl w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              <div className="border-2 border-dashed border-[var(--border-color)] rounded-xl p-8 text-center hover:border-blue-400/50 transition-colors">
              ...
              <div>
                ...
                <input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="e.g., Contract_Draft.pdf"
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button onClick={handleUpload} className="px-4 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 text-sm font-medium">Upload</button>
              </div>
            </div>
          </div>
        </div>
